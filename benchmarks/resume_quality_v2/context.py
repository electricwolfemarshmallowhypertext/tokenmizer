"""Deterministic context construction and the single v2 token counter."""
...
def count_tokens(text: str) -> int:
    """Count every v2 resume context with one fixed local tokenizer."""
    installed = version("tiktoken")
    if installed != TOKEN_COUNTER_VERSION:
        raise RuntimeError(
            f"protocol v2 requires tiktoken {TOKEN_COUNTER_VERSION}, found {installed}"
        )
    if not text:
        return 0
