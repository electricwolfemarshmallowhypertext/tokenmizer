...
the frozen v3 source manifest before and after the process call.
"""
...
from benchmarks.resume_quality_v3.context import (
...
from benchmarks.resume_quality_v3.protocol import (
    CONTINUATION_PROMPT,
    METHODS,
    MODEL_CONFIG,
    ORDER_SEED,
...
from benchmarks.resume_quality_v3.transport import (
    OpenRouterRouteGuard,
...
def _result_affecting_files() -> tuple[Path, ...]:
    ...
    files = {
        ...
        (Path(__file__).parent / "transport.py").resolve(),
...
def _validate_model_config(config: dict) -> None:
    ...
    if config != MODEL_CONFIG:
        raise RuntimeError("model configuration differs from protocol v3")
    if config["model"].lower() in {"auto", "latest", "default"}:
        raise RuntimeError("model must be pinned to an exact model string")
    if config["provider"] != "openrouter":
        raise RuntimeError("protocol v3 provider must be exactly openrouter")
    if config["model"] != "qwen/qwen3-coder":
        raise RuntimeError("protocol v3 model must be exactly qwen/qwen3-coder")
    if config["base_url"].rstrip("/") != "https://openrouter.ai/api":
        raise RuntimeError("protocol v3 base URL must be exactly https://openrouter.ai/api")
    if config["provider_endpoint"] != "deepinfra/turbo":
        raise RuntimeError("protocol v3 endpoint must be exactly deepinfra/turbo")
    if config["provider_allow_fallbacks"] is not False:
        raise RuntimeError("OpenRouter provider fallback must be disabled")
    if config["provider_require_parameters"] is not True:
        raise RuntimeError("OpenRouter must require full parameter support")
    if config["openrouter_metadata"] is not True:
        raise RuntimeError("OpenRouter routing metadata must be enabled")
    if config["transport"] != "local-route-guard-v1":
        raise RuntimeError("protocol v3 transport must use the pinned route guard")
    ...
    if config["single_worker"] is not True or config["parallel_runs"] is not False:
        raise RuntimeError("protocol v3 requires one sequential worker")
    for cache_field in (
        "semantic_cache",
        "response_cache",
...
def _load_lock(suite: Path, *, verify_artifacts: bool = True) -> dict:
    ...
    expected_hash = lock.get("protocol_hash_v3")
    if current_manifest != saved_manifest:
        raise RuntimeError("protocol v3 source manifest changed after freeze")
    if current_hash != expected_hash:
        raise RuntimeError("protocol v3 hash mismatch")
    if (suite / "PROTOCOL_SHA256_V3").read_text(encoding="utf-8").strip() != expected_hash:
        raise RuntimeError("PROTOCOL_SHA256_V3 does not match the lock")
    _validate_model_config(lock.get("model_config", {}))
    if verify_artifacts:
        for relative, expected in lock["artifact_manifest"].items():
            path = suite / relative
            if not path.is_file() or _sha256_file(path) != expected:
                raise RuntimeError(f"frozen v3 artifact changed after lock: {relative}")
    return lock


def _transport_dry_test() -> dict:
    """Capture the exact routed OpenRouter request without network access."""
    ...
    with httpx.Client(transport=httpx.MockTransport(handler)) as client:
        _, audit = send_openrouter_message(client, body, "dry-test-key", MODEL_CONFIG)

    routed = captured.get("body")
    if not isinstance(routed, dict):
        raise RuntimeError("transport dry test did not capture a request body")
    checks = {
        "provider_url_exact": captured["url"] == "https://openrouter.ai/api/v1/messages",
        "model_exact": routed.get("model") == MODEL_CONFIG["model"],
        "provider_endpoint_exact": routed.get("provider") == {
            "only": ["deepinfra/turbo"],
            "order": ["deepinfra/turbo"],
            "allow_fallbacks": False,
            "require_parameters": True,
        },
        "routing_metadata_enabled": captured["headers"].get("x-openrouter-metadata")
        == "enabled",
        "streaming_off": routed.get("stream") is False,
        "system_preserved": routed.get("system") == system_text,
        "user_preserved": routed.get("messages", [{}])[0].get("content") == user_text,
        "tool_call_preserved": routed.get("messages", [{}, {}])[1].get("content", [{}])[0]
        == {
            "type": "tool_use",
            "id": tool_id,
            "name": tool_name,
        ...
        "tool_result_preserved": routed.get("messages", [{}, {}, {}])[2].get(
            "content", [{}]
        )[0]
        == {
            "type": "tool_result",
        ...
        "tool_definition_preserved": routed.get("tools", [{}])[0].get("name") == tool_name,
        "no_cache_control": "cache_control" not in json_canonical(routed),
        "response_model_exact": audit["response_model"] == MODEL_CONFIG["model"],
        "response_provider_exact": audit["provider_name"] == "DeepInfra",
        "provider_usage_returned": isinstance(audit["provider_reported_usage"], dict),
    }
    if not all(checks.values()):
        failed = sorted(name for name, passed in checks.items() if not passed)
        raise RuntimeError(f"transport dry test failed: {failed}")
    return {"status": "pass", "checks": checks, "captured_request": routed}
...
def _validate_config_precedence() -> dict:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="tokenmizer-config-v3-") as temporary:
...
def _require_live_credentials(env: dict[str, str]) -> None:
    ...
    if not openrouter_key:
        raise RuntimeError(
            "OPENROUTER_API_KEY is required for protocol v3"
        )
    if env.get("ANTHROPIC_AUTH_TOKEN") != openrouter_key:
        raise RuntimeError("Claude Code auth token does not match OPENROUTER_API_KEY")
    if env.get("ANTHROPIC_API_KEY") != "":
        raise RuntimeError("ANTHROPIC_API_KEY must be explicitly empty for OpenRouter")
...
def _pre_model_call_guard(
    ...
    if current["protocol_hash_v3"] != lock["protocol_hash_v3"]:
        raise RuntimeError("protocol changed before model call")
    if _claude_version() != lock["claude_code_version"]:
        raise RuntimeError("Claude Code version differs from frozen v3")
...
def _execute_model_call(
    ...
    *,
    ...
    routed_env = env.copy()
    audit_path = Path(routed_env["CLAUDE_CONFIG_DIR"]) / "openrouter-route-audit.json"
    try:
        with OpenRouterRouteGuard(
            routed_env["OPENROUTER_API_KEY"], MODEL_CONFIG, audit_path
        ) as route_guard:
            routed_env["ANTHROPIC_BASE_URL"] = route_guard.base_url
            _PAID_MODEL_CALLS_EXECUTED += 1
            return _run(args, cwd=cwd, env=routed_env, timeout=timeout, check=False)
    finally:
        current = _load_lock(suite)
        if current["protocol_hash_v3"] != lock["protocol_hash_v3"]:
...
def _validation_report(suite: Path, protocol_hash_v3: str) -> dict:
    ...
    return {
        "protocol_version": PROTOCOL_VERSION,
        "protocol_hash_v3": protocol_hash_v3,
...
def freeze_suite(output: Path) -> None:
    if output.exists() and any(output.iterdir()):
        raise FileExistsError(f"refusing to overwrite non-empty v3 suite: {output}")
    output.mkdir(parents=True, exist_ok=True)
    _validate_model_config(MODEL_CONFIG)
    claude_version = _claude_version()
    manifest = _protocol_manifest()
    protocol_hash_v3 = _protocol_hash(manifest)
    snapshot = _protocol_snapshot(claude_version)

    _write_json(output / "protocol.manifest.json", manifest)
    _write_text(output / "PROTOCOL_SHA256_V3", protocol_hash_v3 + "\n")
    ...
    for case in CASES:
        ...
        with tempfile.TemporaryDirectory(prefix="tokenmizer-state-v3-", dir=private_dir) as storage:
        ...
        _write_text(
            ...
            "Generate with protocol v3 summarize; truncation is forbidden.\n",
        )
        _write_json(
    ...
    report = _validation_report(output, protocol_hash_v3)
    ...
    print(f"Frozen {len(CASES)} protocol-v3 cases at {output}")
    print(f"Protocol v3 SHA-256: {protocol_hash_v3}")


def validate_suite(suite: Path) -> dict:
    lock = _load_lock(suite)
    report = _validation_report(suite, lock["protocol_hash_v3"])
...
def live_preflight(suite: Path, output: Path) -> dict:
    """Run one separately recorded OpenRouter transport request."""
    if output.exists():
        raise FileExistsError(f"refusing to overwrite live preflight: {output}")
    lock = _load_lock(suite)
    _validate_model_config(lock["model_config"])
    api_key = os.environ.get("OPENROUTER_API_KEY", "").strip()
    if not api_key:
        raise RuntimeError("OPENROUTER_API_KEY is required for the live preflight")

    system_text = (
        "Protocol v3 live preflight. Call preflight_echo exactly once with token "
        "openrouter-v3. Do not answer with text."
    )
    user_text = "Perform the required transport check now."
    tool = {
        "name": "preflight_echo",
        "description": "Return the exact protocol transport sentinel.",
        "input_schema": {
            "type": "object",
            "properties": {"token": {"type": "string", "enum": ["openrouter-v3"]}},
            "required": ["token"],
    ...
    with httpx.Client(timeout=120) as client:
        response, audit = send_openrouter_message(
            client, request, api_key, MODEL_CONFIG
        )
    ...
    checks = {
        "requested_model_used": payload.get("model") == MODEL_CONFIG["model"],
        "provider_confirmed": str(audit.get("provider_name", "")).lower()
        == "deepinfra",
        "system_preserved": audit["system_preserved"],
        "user_messages_preserved": audit["messages_preserved"],
        "tool_definition_preserved": audit["tools_preserved"],
        "tool_call_preserved": len(tool_calls) == 1
        and tool_calls[0].get("name") == "preflight_echo"
        and tool_calls[0].get("input") == {"token": "openrouter-v3"},
        "usage_returned": isinstance(audit.get("provider_reported_usage"), dict),
        "fallbacks_disabled": audit["fallbacks_disabled"],
        "streaming_off": audit["streaming_off"],
    }
    if not all(checks.values()):
        failed = sorted(name for name, passed in checks.items() if not passed)
        raise RuntimeError(f"live OpenRouter preflight failed: {failed}")

    record = {
        "record_type": "transport_preflight_not_benchmark_result",
        "protocol_hash_v3": lock["protocol_hash_v3"],
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "model": MODEL_CONFIG["model"],
        "provider": MODEL_CONFIG["provider"],
        "provider_endpoint": MODEL_CONFIG["provider_endpoint"],
        "base_url": MODEL_CONFIG["base_url"],
        "checks": checks,
        "request": audit["request"],
        "response": payload,
        "live_preflight_model_calls": 1,
        "summaries_executed": 0,
        "continuations_executed": 0,
        "status": "pass",
    }
    _write_json(output, record)
    post = _load_lock(suite)
    if post["protocol_hash_v3"] != lock["protocol_hash_v3"]:
        raise RuntimeError("protocol changed during live preflight")
    return record
...
def summarize_suite(suite: Path) -> None:
    ...
    if (suite / "summaries.lock.json").exists():
        raise FileExistsError("v3 summaries are already frozen")
    _print_model_config(lock["model_config"])
    ...
    with tempfile.TemporaryDirectory(prefix="tokenmizer-summary-v3-") as temporary:
        root = Path(temporary)
        for case_id in _frozen_case_ids(suite):
            case_dir = suite / "cases" / case_id
            metadata = json.loads((case_dir / "metadata.json").read_text(encoding="utf-8"))
...
def _verify_summary_lock(suite: Path, lock: dict) -> dict:
    ...
    if not path.is_file():
        raise RuntimeError("v3 summaries are not prepared")
    summary_lock = json.loads(path.read_text(encoding="utf-8"))
    if summary_lock.get("protocol_hash_v3") != lock["protocol_hash_v3"]:
...
def _single_run_lock(output_dir: Path) -> Iterator[None]:
    ...
    try:
        ...
        raise RuntimeError("another protocol v3 run is active") from exc
    os.close(descriptor)
    try:
        yield
    finally:
...
def run_suite(suite: Path, output_dir: Path, work_root: Path | None) -> None:
    if os.name == "nt":
        raise RuntimeError("live v3 runs require Claude Code's Linux/WSL2 OS sandbox")
    lock = _load_lock(suite)
    ...
    with _single_run_lock(suite):
        if work_root is None:
            with tempfile.TemporaryDirectory(prefix="tokenmizer-runs-v3-") as temporary:
                root = Path(temporary)
...
def report_suite(suite: Path, results_dir: Path, output: Path | None = None) -> dict:
    ...
    if any(row.get("protocol_hash_v3") != lock["protocol_hash_v3"] for row in results):
    ...
    report = {"protocol_hash_v3": lock["protocol_hash_v3"], "methods": table, "results": results}
    if output:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    freeze = commands.add_parser("freeze", help="locally freeze/hash protocol v3")
    freeze.add_argument("--output", type=Path, required=True)
    validate = commands.add_parser("validate", help="run local-only v3 preflight checks")
    validate.add_argument("--suite", type=Path, required=True)
    preflight = commands.add_parser(
        "live-preflight", help="run one paid OpenRouter transport preflight"
    )
    ...
    report = commands.add_parser("report", help="aggregate exactly 30 v3 results")
    report.add_argument("--suite", type=Path, required=True)
    report.add_argument("--results", type=Path, required=True)
    report.add_argument("--output", type=Path)
    release = commands.add_parser("release-verifiers", help="release v3 hidden verifiers")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "freeze":
        ...
        live_preflight(args.suite.resolve(), args.output.resolve())
