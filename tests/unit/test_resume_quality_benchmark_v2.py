...
from benchmarks.resume_quality_v2 import runner
from benchmarks.resume_quality_v2.context import count_tokens, trim_suffix
from benchmarks.resume_quality_v2.protocol import MODEL_CONFIG, REVIEW_LABELS
...
def test_protocol_manifest_binds_all_result_affecting_surfaces():
    ...
    required = {
        "benchmarks/resume_quality_v2/runner.py",
        "benchmarks/resume_quality_v2/protocol.py",
        "benchmarks/resume_quality_v2/context.py",
    ...
    changed["file_hashes"]["benchmarks/resume_quality_v2/runner.py"] = "0" * 64
...
def test_exact_model_provider_cache_and_execution_config_are_pinned():
    ...
    assert MODEL_CONFIG["base_url"] == "https://api.anthropic.com"
...
def test_single_run_lock_is_exclusive(tmp_path):
    with runner._single_run_lock(tmp_path):
        with pytest.raises(RuntimeError, match="another protocol v2 run is active"):
            with runner._single_run_lock(tmp_path):
                pass
...
def test_freeze_writes_bound_manifest_and_zero_call_validation(monkeypatch, tmp_path):
    monkeypatch.setattr(runner, "_claude_version", lambda: "claude-test-v2")
    monkeypatch.setattr(runner, "_git_sha", lambda: "c" * 40)
    suite = tmp_path / "suite-v2"

    runner.freeze_suite(suite)
    lock = runner._load_lock(suite)
    report = json.loads((suite / "validation_report.json").read_text())

    assert lock["protocol_version"] == "2.0.0"
    assert lock["protocol_hash_v2"] == (suite / "PROTOCOL_SHA256_V2").read_text().strip()
